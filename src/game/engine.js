...
const REAL_ROOMS = ["pod_room", "control_room", "central_hub", "research_lab", "power_station", "maintenance_bay"];
export const ROOM_POD_ROOM = "pod_room";
export const ROOM_CONTROL_ROOM = "control_room";
export const ROOM_CENTRAL_HUB = "central_hub";
export const ROOM_RESEARCH_LAB = "research_lab";
export const ROOM_POWER_STATION = "power_station";
export const ROOM_MAINTENANCE_BAY = "maintenance_bay";
...
const FAILURE_REASON_KEY_BY_ROOM = {
...
export function applyAction(state, runtime, action) {
    ...
    if (action.startsWith("allocation increase:")) {
        adjustAllocation(state, logs, action.slice("allocation increase:".length), 1);
        return { logs };
    }
    if (action.startsWith("allocation decrease:")) {
        adjustAllocation(state, logs, action.slice("allocation decrease:".length), -1);
        return { logs };
    }
...
function lookAroundPodRoom(state, logs) {
    const podRoom = state.rooms.pod_room;
...
function lookAroundControlRoom(state, logs) {
...
function lookAroundHub(state, logs) {
...
function lookAroundResearchLab(state, logs) {
    const room = state.rooms.research_lab;
    if (room.lookStep === 0) {
        room.lookStep = 1;
...
function lookAroundPowerStation(state, logs) {
    const room = state.rooms.power_station;
    if (room.lookStep === 0) {
        room.lookStep = 1;
        room.discovered = true;
...
function lookAroundMaintenance(state, logs) {
    const room = state.rooms.maintenance_bay;
    if (room.lookStep === 0) {
        room.lookStep = 1;
        room.discovered = true;
...
function feelAroundControlRoom(state, logs) {
    const controlRoom = state.rooms.control_room;
...
function feelAroundMaintenance(state, logs) {
    const room = state.rooms.maintenance_bay;
...
function inspectRestrictedDoor(state, logs) {
    if (state.currentRoomId !== ROOM_RESEARCH_LAB) {
        return;
    }
    const room = state.rooms.research_lab;
...
function inspectControlPanel(state, logs) {
    if (state.currentRoomId !== ROOM_POWER_STATION) {
        return;
    }
    const room = state.rooms.power_station;
...
function adjustAllocation(state, logs, roomId, delta) {
    if (state.currentRoomId !== ROOM_POWER_STATION || !state.rooms.power_station.controlPanelInspected) {
        return;
    }
...
function inspectFailureReason(logs, roomId) {
...
function unlockToolkit(state, logs) {
...
function inspectMovement(state, logs) {
...
function repairCables(state, logs) {
    if (state.currentRoomId !== ROOM_MAINTENANCE_BAY) {
        return;
    }
    const room = state.rooms.maintenance_bay;
...
function takeEmergencyToolkit(state, logs) {
    if (state.currentRoomId !== ROOM_MAINTENANCE_BAY) {
        return;
    }
    const room = state.rooms.maintenance_bay;
...
function applyAmbientLogs(state, runtime, deltaMs, rng, logs) {
...
function applyAllocationEffects(state, logs) {
    void logs;
...
function getNavigationState(state) {
    ...
    for (const descriptor of descriptors) {
        ...
        if (descriptor.canEnter) {
            ...
            const edge = getEdge(state.currentRoomId, id, state);
            canEnter = Boolean(edge && edge.state === "open" && isEdgeDiscovered(state, state.currentRoomId, id));
...
function getPowerPanelState(state) {
    const unlocked = state.rooms.power_station.controlPanelInspected;
    const rows = ["pod_room", "control_room", "life_support", "med_bay", "maintenance_bay", "restricted_lab"].map((roomId) => {
        const units = state.power.emergencyAllocation[roomId];
        const locked = roomId === "restricted_lab";
        return {
            id: roomId,
            label: roomId === "restricted_lab"
                ? "Restricted Lab"
                : roomId === "pod_room"
...
function getMapPanelState(state) {
    return { visible: false, text: "" };
}
function getCurrentRoomDisplayName(state) {
...
function getRoomLabelForNav(roomId, state) {
    if (roomId === "pod_room") {
        return state.rooms.pod_room.displayName;
    }
    if (roomId === "control_room") {
...
function getEdgesForRoom(roomId, state) {
...
function controlRoomVisualsActive(state) {
...
function getReturnLogKeyForPodRoom(state) {
...
function getReturnLogKeyForControlRoom(state) {
...
function getReturnLogKeyForResearchRoom(state) {
